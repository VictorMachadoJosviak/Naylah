...
{
    public class EntryRenderer : global::Xamarin.Forms.Platform.Android.AppCompat.ViewRenderer<EntryBase, EditText>
    {
        ...
        {
            NativeView.InputType = Element.IsPassword ? InputTypes.TextVariationPassword | InputTypes.ClassText : NativeView.InputType;
        }
        ...
        {
            NativeView.Enabled = Element.IsEnabled;
        }

        private EditText InitializeNativeView()
        {
            var view = FindViewById<EditText>(Naylah.Xamarin.Android.Resource.Id.editText);

            view.TextChanged += EditTextOnTextChanged;
            view.FocusChange += EditText_FocusChange;
        ...
        protected override EditText CreateNativeControl()
        {
            if (Element.Theme == BasicTheme.Dark)
            {
                if (IsNumeric)
                {
                    return (EditText)LayoutInflater.From(Context).Inflate(Naylah.Xamarin.Android.Resource.Layout.HandleCustomEditText, null);
                }
                else
                {
                    return (EditText)LayoutInflater.From(Context).Inflate(Naylah.Xamarin.Android.Resource.Layout.EditText, null);
                }
            }
            else
            {
                if (IsNumeric)
                {
                    return (EditText)LayoutInflater.From(Context).Inflate(Naylah.Xamarin.Android.Resource.Layout.HandleCustomEditTextLight, null);
                }
                else
                {
                    return (EditText)LayoutInflater.From(Context).Inflate(Naylah.Xamarin.Android.Resource.Layout.EditTextLight, null);
                }
            }
        }
